            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0">List of Administrator</h5>
                    </div>
                    <div class="card-body">
                    <div class="d-flex justify-content-between">
                    <div class="form-group col-md-6">
                        <input type="text" class="form-control" v-model="tableData.search"  @input="listOfAdministrator()" placeholder="Search....">
                    </div>
                    <div class="btn-group">
                    ...
                    <data-table class="mt-2" :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy">
                        <tbody>
                            <tr class="tr-shadow" v-for="(list, idx) in administrators" :key="idx">
                                <td class="text-primary">
                                    <strong>{{ list.last_name }}, {{list.first_name}} {{ list.middle_name }}
                                    </strong>
                                    </td>
                                <td class="desc">{{ extractGender(list.gender) }}</td>
                                <td class="desc">{{ list.role == 1 ? "Supervisor" : "HR" }}</td>
                            ...
                            <tr> 
                                <td colspan="6" v-show="!noData(administrators)">
                                    No Result Found!
                                </td>
                            </tr>
    ...
    methods: {
        ...
        saveUser(){
            if(this.post.id > 0){
                this.$axios.get('sanctum/csrf-cookie').then(response=>{
                    this.btncap = "Saving...";
                    this.$axios.put('api/administrator/'+this.post.id, this.post).then(res=>{
                        this.btncap = "Save";
                ...
                this.$axios.get('sanctum/csrf-cookie').then(response=>{
                    this.btncap = "Saving...";
                    this.$axios.post('api/administrator', this.post).then(res=>{
                        this.btncap = "Save";
        ...
        listOfUser(urls='api/administrator'){
            this.$axios.get('sanctum/csrf-cookie').then(response => {
                this.tableData.draw ++;
                this.$axios.get(urls,{params:this.tableData}).then(res=>{
                let data = res.data;
                    if(this.tableData.draw == data.draw){
                        this.administrators = data.data.data;
